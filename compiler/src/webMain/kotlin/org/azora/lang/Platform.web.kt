...
/** Web/WASM implementation: no-op - Wasm is single-threaded, so no locking is needed. */
internal actual inline fun <R> azSync(lock: Any, block: () -> R): R = block()

/**
 * Web/WASM implementation: there is no way to block the single-threaded Wasm event loop, so this
...
): T = error("runBlocking is unavailable on Wasm; use IrInterpreter.interpretSuspend()")
