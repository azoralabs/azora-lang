...
class Parser(
     ...
     */
    private val REPEAT_COUNT_NAME = "count"
    ...
    fun parse(): Program {
        ...
        // A decorator above the header decorates the *module* - the `.Module`
        // target. The same rows above anything else belong to the declaration
        // that follows, so they are read here only when a header really does
        // follow; otherwise the scan is rewound and the ordinary top-level
        // parse sees them where it expects to.
        val beforeAnnotations = current
        val leadingAnnotations = if (check(TokenType.AT)) parseAnnotations() else emptyList()
        val moduleAnnotations = if (isModuleHeaderAhead()) leadingAnnotations else emptyList()
        if (leadingAnnotations.isNotEmpty() && moduleAnnotations.isEmpty()) current = beforeAnnotations
    ...
    /** The keyword a binding was written with, for a diagnostic that quotes it back. */
    private fun bindingKeywordOf(item: TopLevel): String? = when (item) {
        is TopLevel.VarDecl -> if (item.valueMutable) "var" else "val"
        is TopLevel.LetDecl -> "let"
        else -> null
    }
    ...
    private fun parseReceiverAndBody(): Triple<ParamModifier, List<Param>, List<Stmt>> {
        ...
        if (match(TokenType.L_BRACE)) {
            ...
            while (!check(TokenType.R_BRACE) && !isAtEnd()) { body.addAll(parseStmts()); skipNewlines() }
    ...
    private fun parseTopLevel(): TopLevel {
        ...
        return when {
            ...
            // `direct spec Number { prop rank: Int }` - a spec whose members
            // belong to the type rather than to its values. This is how a
            // type-side implementation is declared, in place of the `impl Spec
            // for Type::` that used to say it at the implementing end.
            check(TokenType.DIRECT) && peekNext()?.type == TokenType.SPEC -> {
                advance(); parseSpec(isDirect = true)
            }
        ...
        if (!check(TokenType.R_PAREN)) {
            do {
                ...
                if (check(TokenType.DOT) && peekNext()?.type == TokenType.IDENTIFIER) {
                    ...
                    advance() // ':' or '='
                    // A named argument takes the same shorthand its positional
                    // twin does: the field states the type, so `.Unused` names
                    // the case without repeating it.
                    if (check(TokenType.DOT) && peekNext()?.type == TokenType.IDENTIFIER) {
                        val dot = advance()
                        val member = advance().lexeme
                        named.add(key to Expr.Identifier(".$member", dot.line, dot.column))
                    } else {
    ...
    private fun parseDecoTargets(): Set<DecoTarget> {
        ...
        // `for .*` is every target, which is what a decorator with no `for`
        // already means: an empty set. Spelling it out is allowed because a
        // reader may want to see the intent, and it says the same thing.
        if (check(TokenType.DOT) && peekNext()?.type == TokenType.STAR) {
            advance()
            advance()
            return emptySet()
        }
    ...
    private fun parseDeco(annotations: List<Annotation>, isBridge: Boolean = false): TopLevel.Deco {
        val start = peek()
        consume(TokenType.ANNOT, "Expected 'annot'")
        // A decorator is declared as it is written. `annot Serializable` named
        // one thing at its declaration and another at every use site, and the
        // `@` is what tells a reader which of the two a name is.
        if (!check(TokenType.AT)) {
            val written = peek()
            error("Decorator declarations carry their '@': use 'annot @${written.lexeme}' at line ${written.line}")
        }
        advance()
    ...
    private fun foldBraceInterpolation(
        ...
        while (k < tokens.size) {
            ...
            if (t.type == TokenType.IDENTIFIER && t.lexeme == "$" &&
                ...
                folded = foldConstantArithmetic(folded)
    ...
    /**
     * Folds a splice that is arithmetic over integers into the number it is.
     *
     * `${8 << index}` is a width, and a width has to arrive as one: a type
     * argument, a doc line and a field count all want the value, not three
     * tokens that would add up to it. Anything that is not integer arithmetic
     * is left exactly as written - a splice may name something instead.
     */
    private fun foldConstantArithmetic(tokens: List<Token>): List<Token> {
        val value = ConstantFolder(tokens).evaluate() ?: return tokens
    ...
    /**
     * The one arithmetic expression a splice may hold, read by precedence.
     *
     * Shifts bind loosest, then `+`/`-`, then `*`/`/`/`%` - the order every
     * language with these operators uses, and the order a reader of
     * `${8 << index + 1}` expects.
     */
    private class ConstantFolder(private val tokens: List<Token>) {
        private var at = 0

        fun evaluate(): Long? {
            val value = shift() ?: return null
            return if (at == tokens.size) value else null
        }

        private fun shift(): Long? {
            var left = sum() ?: return null
            while (true) {
                val op = peek()?.type
                if (op != TokenType.SHIFT_LEFT && op != TokenType.SHIFT_RIGHT) return left
                at++
                val right = sum() ?: return null
                left = if (op == TokenType.SHIFT_LEFT) left shl right.toInt() else left shr right.toInt()
            }
        }
        ...
        private fun sum(): Long? {
            ...
            while (true) {
                val op = peek()?.type
                if (op != TokenType.PLUS && op != TokenType.MINUS) return left
                at++
                val right = product() ?: return null
                left = if (op == TokenType.PLUS) left + right else left - right
            }
        }

        private fun product(): Long? {
            var left = atom() ?: return null
            while (true) {
                val op = peek()?.type
                if (op != TokenType.STAR && op != TokenType.SLASH && op != TokenType.PERCENT) return left
                at++
                val right = atom() ?: return null
                if ((op == TokenType.SLASH || op == TokenType.PERCENT) && right == 0L) return null
                left = when (op) {
        ...
        private fun atom(): Long? {
            ...
            return when (token.type) {
                TokenType.MINUS -> { at++; atom()?.let { -it } }
                TokenType.INT_LITERAL -> { at++; (token.literal as? NumericLiteral)?.value as? Long }
                TokenType.L_PAREN -> {
                    at++
                    val inner = shift() ?: return null
                    if (peek()?.type != TokenType.R_PAREN) return null
                    at++
                    inner
                }
                else -> null
    ...
    private fun isTypeListBindingAhead(): Boolean {
        ...
        // `Array<Elem>` / `Array<Elem>` - an inline compile-time value list.
    ...
    private fun parseTypeListBinding(): TopLevel {
        ...
        if (check(TokenType.L_BRACKET)) {
            ...
            if (type !is TypeRef.Named || type.name != "Array") {
                error("Expected 'Array<…>' or '[Elem]' compile-time list type at line ${start.line}")
            }
        }
    ...
    private fun parseComptimeListTerm(): List<String> {
        ...
        if (match(TokenType.L_BRACKET)) {
            ...
            if (!check(TokenType.R_BRACKET)) {
                do {
                    values.add(when {
                        // A bracket is a group of whatever is written in it: a
                        // list of types, or a list of values. `@arr[…]` builds
                        // an array at run time; a compile-time list needs no
                        // macro to say what it plainly is.
                        ...
                        // `std` must still write `Int`. Dropping it would
    ...
    private fun parseTopLevelDeepInline(): TopLevel {
        return when (peekNext()?.type) {
            TokenType.FUNC -> {
                advance()
                TopLevel.Func(parseFuncDecl(isInline = true))
            }
    ...
    /**
     * `[exposed|confined] test scope { … }` - a scope of test-only declarations.
     *
     * Whatever is declared inside is ordinary code, and is emitted as such; what
     * the scope changes is who may refer to it. Only a `test` block, or another
     * declaration in a test scope, may - so a fixture cannot leak into the program
     * it exists to test.
     *
    ...
    private fun parsePack(
        ...
        // A primitive is written rather than built, and this says which literal
        // writes it. Only a bridge pack may: these are the compiler's own
        // types, and nothing a library declares is a literal.
        val literalKind = if (check(TokenType.L_PAREN)) {
            if (!isBridge) {
                error(
                    "'$keyword $name(…)' is a bridge form at line ${start.line}: " +
                        "only the compiler's own types are written as literals",
                )
            }
            advance()
            val written = advance()
            val kind = written.lexeme
            if (kind !in Literals.KINDS) {
                error(
                    "'$kind' is not a literal at line ${start.line}: " +
                        "write ${Literals.KINDS.joinToString(" or ") { "'$it'" }}",
                )
            }
            consume(TokenType.R_PAREN, "Expected ')' after the literal a bridge pack is written as")
            kind
        } else {
            null
        }
    ...
    private fun derivedImpl(
        ...
        decoratorArgs: List<Expr> = emptyList(),
        decoratorNamedArgs: List<Pair<String, Expr>> = emptyList(),
    ...
    private fun parseEnumDecl(annotations: List<Annotation> = emptyList()): TopLevel.Enum {
        ...
        // `enum Name<T>` - parameters after the name they parametrize, as everywhere.
        val enumTypeParams = parseTypeParams().names
        ...
        return TopLevel.Enum(name, variants, start.line, start.column, annotations, variantAnns, enumTypeParams)
    }
    ...
    private fun parseFailDecl(annotations: List<Annotation> = emptyList()): TopLevel.Fail {
        ...
        // `error Name<T>` - an error set may be generic over what its cases carry.
        val failTypeParams = parseTypeParams().names
        ...
        return TopLevel.Fail(
            name, variants, start.line, start.column, annotations, variantAnns, variantPayloads,
            typeParams = failTypeParams,
        )
    }
    ...
    private fun parseTypeArg(): TypeRef = when {
        ...
        // `add<Short, _, Long>(1, 2, 3)` - a hole says "work this one out",
        // which is what an unwritten list says about every position. Writing
        // some of them is how a caller pins the ones that matter and leaves
        // the rest to the arguments.
        check(TokenType.IDENTIFIER) && peek().lexeme == TypeRef.HOLE -> {
            advance()
            TypeRef.Named(TypeRef.HOLE)
        }
        // `tupleOf<Int, ...T>(…)` - a type pack spread among the type
    ...
    private fun parseImpl(isBridge: Boolean = false, annotations: List<Annotation> = emptyList()): TopLevel.Impl {
        ...
        // its receiver inside the braces - the last form that did either. A
        // constructor is a member, so it is written where every other member
        // is, and the error says so rather than only that this is wrong.
        ...
        if (check(TokenType.CTOR)) {
            ...
            skipNewlines()
            // The receiver belongs to the signature. This form predates that rule and
            // is removed whole in its own step, so its in-brace receiver is optional
            // here rather than an error - the body alone is the modern spelling.
            val receiverModifier = if (isSelfReceiverHeaderAhead()) {
                val bound = parseReceiverBinding("ctor receiver").modifier
        ...
        if (check(TokenType.DTOR)) {
            ...
            skipNewlines()
            // Optional for the same reason the ctor form's is - see above.
            val receiverModifier = if (isSelfReceiverHeaderAhead()) {
                val bound = parseReceiverBinding("dtor receiver").modifier
        ...
        while (!check(TokenType.R_BRACE) && !isAtEnd()) {
            ...
            when {
                check(TokenType.PROP) -> {
                    ...
                    val propReceiver = prefixReceiver
                    ...
                    // reading the body. A local binding may infer, because its
                    // initializer is right beside it; a member may not.
                    if (!propTypeDeclared) {
                        error(
                            "property '$propName' must declare its type at line ${methodStart.line}: " +
                                "write 'prop $propName[…]: T = …'",
                        )
                    }
                ...
                check(TokenType.BRIDGE) -> {
                    ...
                    when {
                        ...
                        check(TokenType.PROP) -> {
                            ...
                            if (check(TokenType.L_BRACKET)) {
                                error("bridge property receivers no longer use brackets; write 'bridge prop &.$propName: T' at line ${peek().line}")
                            }
                            val bridgeReceiver = prefixReceiver
                                ?: PropReceiver("self", TypeRef.Named("Self"), ParamModifier.SHARED, declared = false)
                        ...
                        check(TokenType.FUNC) -> {
                            ...
                            consumeNewline()
                            methods.add(FuncDecl(fnName, params, ret, emptyList(), bridgeInline, tp.names, methodStart.line, methodStart.column, annotations = memberAnnotations, visibility = visibility, receiverModifier = prefixReceiver?.modifier ?: ParamModifier.SHARED, receiverName = prefixReceiver?.name ?: "self", declaresReceiver = prefixReceiver != null, isDeepInline = bridgeDeep))
                        }
                ...
                check(TokenType.CTOR) -> {
                    ...
                    } else if (check(TokenType.AMP) || check(TokenType.BANG)) {
                        error("constructors cannot borrow their result; use 'ctor .(…)' at line ${peek().line}")
                    } else {
                        PropReceiver("self", TypeRef.Named("Self"), ParamModifier.SHARED, declared = false) to emptyList<Param>()
                    }
                    val ctorParams = if (match(TokenType.L_PAREN)) {
                        val parsed = if (check(TokenType.R_PAREN)) emptyList() else parseParams()
                    ...
                    val repeated = if (match(TokenType.STAR)) {
                        val bound = consumeIdentifierLike("Expected 'count' after '*' in a ctor declaration")
                        if (bound != REPEAT_COUNT_NAME) {
                            error(
                                "a repeated ctor binds its repetition as '$REPEAT_COUNT_NAME' at " +
                                    "line ${ctorStart.line}, not '$bound'",
                            )
                        }
                        true
                    } else false
                    ...
                    val repeatContract = if (!repeated) emptyList() else listOf(
                        Stmt.Assert(
                            Expr.Binary(
                                Expr.Identifier(REPEAT_COUNT_NAME, ctorStart.line, ctorStart.column),
                                TokenType.GREATER_EQUAL,
                                Expr.IntLiteral(1, ctorStart.line, ctorStart.column),
                                ctorStart.line, ctorStart.column,
                            ),
                            Expr.StringLiteral(
                                "$REPEAT_COUNT_NAME must be at least 1",
                                ctorStart.line, ctorStart.column,
                            ),
                            ctorStart.line, ctorStart.column,
                        ),
                    )
                    val repeatParam = if (!repeated) emptyList() else listOf(
                        Param(REPEAT_COUNT_NAME, TypeRef.Named("Int")),
                    )
                    methods.add(FuncDecl(
                        "ctor", scopeReceivers + ctorParams + repeatParam, ctorReturn,
                        repeatContract + applyContracts(ctorBody, contracts),
                        false, ctorTypeParams.names, ctorStart.line, ctorStart.column,
                        annotations = memberAnnotations, visibility = visibility,
                        receiverModifier = recv.modifier, receiverName = recv.name,
                        declaresReceiver = recv.declared,
                ...
                check(TokenType.DTOR) -> {
                    ...
                    val recv = if (dotReceiver) {
                        consume(TokenType.L_PAREN, "Expected '()' after 'dtor .'")
                        consume(TokenType.R_PAREN, "A destructor takes no ordinary parameters")
                        PropReceiver("self", TypeRef.Named("Self"), ParamModifier.EXCLUSIVE)
                    } else {
                        if (check(TokenType.L_BRACKET) || check(TokenType.AMP) || check(TokenType.BANG)) {
                            error("destructors cannot borrow their receiver; use 'dtor .()' at line ${peek().line}")
                        }
                        consume(TokenType.L_PAREN, "Expected '()' for a static destructor or '.()' for an instance destructor")
                ...
                check(TokenType.FIN) || check(TokenType.VAR) || check(TokenType.VAL) || check(TokenType.LET) -> {
                    ...
                    pendingTopLevels.add(mangleTopLevel(parseTopLevelBlockItem(deepInline = isInline), typeName))
                }
                else -> error(
                    "Expected 'prop', 'func', 'async prop', 'async func', 'oper', 'ctor', " +
                        "'dtor', 'fin', 'var', 'val', 'let', or 'bridge' in impl block " +
                        "at line ${peek().line}",
                )
            }
            skipNewlines()
    ...
    /**
     * The target of a `bridge`, which is always a `Target` value.
     *
    ...
    /**
     * The members of `Target`, which is what a `bridge` target must name.
     *
    ...
    private fun parseBridgeFunc(annotations: List<Annotation> = emptyList()): TopLevel.Bridge {
        ...
        // `bridge func<T> fill`); capture them so a generic return type like
    ...
    /**
     * `solo Name { fields }` - a pack there is one of.
     *
     * A singleton is declared and implemented exactly as a pack is: fields in the
     * body, members in `impl Name { … }`. The one difference is how many there
     * are - a single instance, ready when the program starts - so `solo` is a
     * word about lifetime, and everything else about the type reads the same.
     *
     * `solo pack Name { … }` is the older spelling. The `pack` said nothing the
     * body had not already said, and is optional.
     */
    private fun parseSolo(visibility: Visibility = Visibility.PUBLIC, annotations: List<Annotation> = emptyList()): TopLevel.Solo {
        ...
        val fields = mutableListOf<PackField>()
        val methods = mutableListOf<FuncDecl>()
        // `solo pack Alloc` - a singleton that carries no state. Its members live
        // in an `impl` like any other type's, so with no fields there is nothing
        // left for a body to hold and the braces are omitted.
        if (!check(TokenType.L_BRACE)) {
            consumeNewline()
    ...
    private fun parseGraph(): TopLevel.Graph {
        ...
        if (match(TokenType.INCLUDES)) {
            if (match(TokenType.L_PAREN)) {
                ...
                consume(TokenType.R_PAREN, "Expected ')' after the includes tuple")
            } else {
        ...
        while (!check(TokenType.R_BRACE) && !isAtEnd()) {
            ...
            val lifetime = when {
                ...
                match(TokenType.SCOPED) -> TopLevel.ProviderLifetime.SCOPED
                // `scope` is a namespace; the lifetime that used to borrow the
                // word now has one of its own.
                check(TokenType.SCOPE) -> error(
                    "A graph's per-scope lifetime is written 'scoped' at line ${entry.line}; " +
                        "'scope' declares a namespace"
                )
                else -> error(
                    "A 'graph' entry starts with its lifetime at line ${entry.line} - " +
                        "'solo' for one per graph, 'factory' for one per resolution, " +
                        "'scoped' for one per active scope"
                )
            }
            ...
            if (match(TokenType.BINDS)) {
                if (match(TokenType.L_PAREN)) {
                    ...
                    consume(TokenType.R_PAREN, "Expected ')' after the binds tuple")
                } else {
    ...
    private fun parseSlot(annotations: List<Annotation> = emptyList()): TopLevel.Slot {
        ...
        // `variant enum Option<T>` - a tagged union is generic over what its cases
        // carry, so the parameters go where every other declaration puts them.
        val slotTypeParams = parseTypeParams().names
        ...
        consumeNewline()
        return TopLevel.Slot(
            name, variants, start.line, start.column, annotations,
            isError = kind == "error", typeParams = slotTypeParams,
        )
    }
     ...
     */
    companion object {
        /** The receiver's name, everywhere it is written. */
        const val SELF_RECEIVER = "self"

        /** `[&]` - a receiver the member takes and does not use. */
        const val IGNORED_RECEIVER = "_"
         ...
         *
         * `macro @compose $w => inline "using composeIn($w)"` is a fragment that
         * names what it was handed, so `@compose world` can become
         * `using composeIn(world)` rather than only ever standing for one text.
         */
    ...
    private fun parseMeta(): TopLevel.Meta {
        ...
        if (match(TokenType.FAT_ARROW)) {
            ...
            // A fragment naming its hole (`"using composeIn($w)"`) lexes as an
        ...
        // `macro @name <pattern> => <template>` - one arm, written where it is.
        // The braces are how several arms are told apart from each other, so a
        // macro with one has nothing for them to do:
        //
        //     macro @arr[...$items] => std.container.array::Array(...$items)
        //     macro @arr { [...$items] => std.container.array::Array(...$items) }
        //
        // A `{` here always opens the arm list, never a brace-delimited pattern -
        // that arm is written inside the list like any other.
        if (!check(TokenType.L_BRACE)) {
            parseTypeMacroArm(name)?.let { typeArm ->
                pendingTypeMacroRules.add(typeArm)
                return TopLevel.Meta(name, emptyList(), start.line, start.column)
            }
            val arm = parseMacroArm()
            consumeNewline()
            return TopLevel.Meta(name, listOf(arm), start.line, start.column)
        }
        ...
        // Arms are grouped by the sigil they answer to, keyed by the suffix that
        // sigil puts on the name - "" for the bare form.
        val armsBySigil = linkedMapOf<String, MutableList<MacroArm>>()
        while (!check(TokenType.R_BRACE) && !isAtEnd()) {
            skipNewlines()
            if (check(TokenType.R_BRACE)) break
            ...
            armsBySigil.getOrPut(macroArmSigil()) { mutableListOf() }.add(parseMacroArm())
        }
        consume(TokenType.R_BRACE, "Expected '}' after macro arms")
        consumeNewline()
        // `@vec` and `@vec!` are still different macros - the sigil is part of the
        // name. Writing their arms in one block only saves declaring the name
        // twice, so each sigil leaves with the arms addressed to it.
        val bare = armsBySigil.remove("") ?: mutableListOf()
        val sigilled = armsBySigil.map { (sigil, sigilArms) ->
            TopLevel.Meta(name + sigil, sigilArms, start.line, start.column, parameter)
        }
        if (bare.isEmpty() && sigilled.isNotEmpty()) {
            pendingTopLevels.addAll(sigilled.drop(1))
            return sigilled.first()
        }
        pendingTopLevels.addAll(sigilled)
        return TopLevel.Meta(name, bare, start.line, start.column, parameter)
    }

    /**
     * The sigil an arm answers to, consumed if present.
     *
     * `![…] => …` inside `macro @vec { … }` is the arm `@vec!` takes. Only a sigil
     * that actually opens an arm is taken, so nothing else beginning with one of
     * these characters is mistaken for a sigil.
     */
    private fun macroArmSigil(): String {
        val sigil = when (peek().type) {
            TokenType.BANG -> "!"
            TokenType.QMARK -> "?"
            TokenType.AMP -> "&"
            TokenType.STAR -> "*"
        ...
        val opensArm = peekNext()?.type.let {
            it == TokenType.L_PAREN || it == TokenType.L_BRACKET || it == TokenType.L_BRACE
        }
        if (!opensArm) return ""
        advance()
        return sigil
    ...
    /**
     * Rejects `use`, which the language no longer has, where it used to be read.
     *
     * The word lexes as an ordinary identifier now, so without this it would
     * reach the newline check and report a mismatched token - true, but no help
     * to somebody reading a file written before the word was dropped. [remedy]
     * is what to write instead.
     */
    private fun rejectRemovedUse(remedy: String) {
        if (!check(TokenType.IDENTIFIER) || peek().lexeme != "use") return
        error("'use' is not a keyword at line ${peek().line}; $remedy")
    }
    ...
    private fun parseMacroArm(): MacroArm {
        ...
        // An arm ends at a newline or at the `}` closing the arm list - and, for
        // the braceless single-arm form, at the end of the file.
    ...
    private fun parseSpec(isBridge: Boolean = false, isDirect: Boolean = false): TopLevel.Spec {
        val start = peek()
        ...
        if (check(TokenType.L_BRACKET)) {
            ...
            // `use` is no longer a word the language has, so the compact
            // one-line spec has no way to name itself and the block form is the
            // only spelling left. Caught here rather than left to the newline so
            // the error names the replacement.
            rejectRemovedUse(
                "a spec's call-site name is declared inside the spec: " +
                    "'spec Into<T> { prop<T> into[self: Self&]: T; " +
        ...
        // `spec Copy requires (Clone, Other)` - capabilities an
        ...
        if (check(TokenType.REQUIRES) &&
            ...
            if (match(TokenType.L_PAREN)) {
                ...
                consume(TokenType.R_PAREN, "Expected ')' to close the 'requires' tuple")
                if (requiredSpecs.size == 1) {
                    error("A single requirement is written 'requires ${requiredSpecs.first().displayName()}', without parentheses, at line ${peek().line}")
                }
            } else {
                requiredSpecs.add(parseTypeName())
            }
        ...
        if (!check(TokenType.L_BRACE)) {
            consumeNewline()
            return TopLevel.Spec(name, emptyList(), start.line, start.column, typeParams = typeParams.names, parents = parentSpecs, requires = requiredSpecs, isBridge = isBridge, isDirect = isDirect, typeDefaults = typeParams.typeDefaults, assocNames = assocNames)
        }
        consume(TokenType.L_BRACE, "Expected '{' after spec name")
        skipNewlines()
        // Inside a spec, `Self` is the type that will implement it - as settled
        // a meaning as it has inside an `impl`, so a member may write `[self&]`
        // here too and leave the type to the one place that already says it.
        val savedImplTypeName = currentImplTypeName
        currentImplTypeName = name
        val methods = mutableListOf<FuncDecl>()
        while (!check(TokenType.R_BRACE) && !isAtEnd()) {
            skipNewlines()
            if (check(TokenType.R_BRACE)) break
            ...
            val mreceiver = prefixReceiver ?: if (check(TokenType.L_BRACKET)) {
                error("spec function receivers no longer use brackets; write 'func &.$mname(…)' at line ${peek().line}")
            } else {
                null
            }
            consume(TokenType.L_PAREN, "Expected '('")
        ...
        currentImplTypeName = savedImplTypeName
        consumeNewline()
        return TopLevel.Spec(name, methods, start.line, start.column, typeParams = typeParams.names, parents = parentSpecs, requires = requiredSpecs, isBridge = isBridge, isDirect = isDirect, typeDefaults = typeParams.typeDefaults, assocNames = assocNames)
    }
    ...
    private fun parseAssocNames(): List<String> {
        ...
        consume(TokenType.R_PAREN, "Expected ')' after the associated type names")
        return names
    }
    ...
    private fun parseAssocBindings(): Map<String, TypeRef> {
        ...
        consume(TokenType.R_PAREN, "Expected ')' after the associated types")
        return bindings
    }
    ...
    private fun parseFuncDecl(
        ...
        // Generic parameters qualify the declaration itself, so they follow
        // `func` and precede an optional receiver: `func<T> T&.map(…)` or,
        // inside an impl, `func<T> &.map(…)`.
        val typeParamsBefore = parseTypeParams()
        val prefixReceiver = if (currentImplTypeName != null) {
        ...
        val parsedTypeParams = typeParamsBefore
        ...
        // in rather than written at it, so they lead the parameters.
        var scopeReceivers = pendingMemberContextReceivers
        ...
        return FuncDecl(
            ...
            constDefaults = parsedTypeParams.constDefaults,
    ...
    /**
     * Consumes the `scope` that introduces a contract-style body.
     *
     ...
     * ```
     *
     * `scope` only introduces a body when a `{` follows it immediately - otherwise
     * it is the block expression of the same name, and belongs to the body itself.
     */
    private fun skipScopeBodyIntroducer() {
        val i = nextMeaningfulIndex()
        if (tokens.getOrNull(i)?.type != TokenType.SCOPE) return
        if (tokens.getOrNull(i + 1)?.type != TokenType.L_BRACE) return
        while (current < i) advance() // newlines between the clause and `scope`
    ...
    private fun parseTypeName(): TypeRef {
        ...
        val base = parseTypeInfixes(parseTypeSuffixes(parseTypeAtom()))
     ...
     *
     * This is called only immediately after the `&` or `!` type sigil, so a
     * single [TokenType.PIPE] starts an origin fence rather than a bitwise-or
     * expression. Empty when no fence is written; an explicitly empty fence is
     * invalid because it promises no usable origin and is indistinguishable in
     * intent from omitting the fence.
     */
    ...
    private fun parseBorrowOrigins(): List<String> {
        if (check(TokenType.L_BRACKET)) {
            error(
                "Borrow-origin brackets were removed; replace '[source]' with " +
                    "'|source|' at line ${peek().line}",
            )
        }
        if (match(TokenType.OR_OR)) {
            error("A borrow-origin fence cannot be empty; omit '||' at line ${peek().line}")
        }
        if (!match(TokenType.PIPE)) return emptyList()
        ...
        consume(TokenType.PIPE, "Expected '|' after the borrow origin list")
    ...
    private fun parseTypeAtom(): TypeRef {
        return when {
            ...
            check(TokenType.ESCAPING) &&
                peekNext()?.type in setOf(TokenType.L_PAREN, TokenType.L_BRACKET, TokenType.IDENTIFIER) ->
                parseCallableType()
            check(TokenType.L_BRACKET) && isCallableTypeAhead() -> error(
                "callable receiver types use a dot receiver head; write 'Type&.(Args) -> Return' at line ${peek().line}",
            )
            check(TokenType.L_BRACKET) -> {
            ...
            check(TokenType.L_PAREN) -> {
                ...
                if (match(TokenType.DOT)) {
                    if (elements.isEmpty()) {
                        error("A callable receiver group cannot be empty at line ${start.line}")
                    }
                    parseCallableTail(elements)
    ...
    /**
     * Recognizes the removed bracketed callable receiver form so it can receive
     * a targeted migration diagnostic instead of being read as a type list.
     *
    ...
    private fun isAsyncCallableTypeAhead(): Boolean {
        ...
        return when (peekNext()?.type) {
            TokenType.L_PAREN, TokenType.IDENTIFIER -> true
    ...
    /** Reads a prefixed callable and applies the prefix to the parsed function. */
    private fun parseCallableType(kind: CallableKind = CallableKind.FUNC): TypeRef {
        val escaping = check(TokenType.ESCAPING)
        if (escaping) advance()
        if (check(TokenType.L_BRACKET) && isCallableTypeAhead()) {
            error(
                "Bracketed callable receivers were removed at line ${peek().line}; " +
                    "replace '[Context&](Args) -> Result' with 'Context&.(Args) -> Result'",
            )
        }
        val parsed = parseTypeName()
        val function = parsed as? TypeRef.Function
            ?: error("Expected a callable type after '${if (escaping) "escaping" else kind.surfaceName}' at line ${peek().line}")
        return function.copy(kind = kind, isEscaping = escaping)
    }

    /** The `(parameters) -> result` tail following a receiver type and dot. */
    private fun parseCallableTail(
        receivers: List<TypeRef>,
        kind: CallableKind = CallableKind.FUNC,
        escaping: Boolean = false,
    ): TypeRef.Function {
        consume(TokenType.L_PAREN, "Expected '(' after the callable receiver dot")
    ...
    private fun parseStmt(): Stmt {
        ...
        return when {
            ...
            // that brings it in is written beside the use. It is still an
            // import, so it leaves here for the top level, where imports are
            // resolved.
            check(TokenType.IMPORT) -> {
                val at = peek()
                pendingTopLevels.add(parseUse())
            ...
            check(TokenType.L_BRACE) -> parseBareGroupAssignment()
                ...
                // says nothing about why the word is not allowed to be there.
                parseExprStmt()
            }
        }
    }
    ...
    private fun purgeCall(value: Expr, start: Token): Expr =
        Expr.Call("__purge", listOf(value), start.line, start.column, start.lexeme.length)
    ...
    private fun parseUsingContext(): Stmt.UsingContext {
        val start = consume(TokenType.USING, "Expected 'using'")
        val values = mutableListOf<Expr>()
        if (check(TokenType.L_BRACKET)) {
            error(
                "Context receiver lists use parentheses at line ${start.line}; " +
                    "replace 'using [a, b] { ... }' with 'using (a, b) { ... }'",
            )
        }
        if (match(TokenType.L_PAREN)) {
            skipNewlines()
            if (check(TokenType.R_PAREN)) {
                error("A contextual receiver group cannot be empty at line ${start.line}")
            }
            while (!check(TokenType.R_PAREN) && !isAtEnd()) {
                val savedTrailing = allowTrailingLambda
                allowTrailingLambda = false
                ...
                when {
                    ...
                    check(TokenType.NEWLINE) -> skipNewlines()
                    check(TokenType.R_PAREN) -> Unit
                    separatedByPhysicalLine -> Unit
                    else -> error("Expected ',' or a new line between contextual receivers")
                }
            }
            consume(TokenType.R_PAREN, "Expected ')' after contextual receivers")
            if (values.size < 2) {
                error(
                    "A single contextual receiver is written without parentheses at line ${start.line}: " +
                        "use 'using value { ... }'",
                )
            }
        } else {
        ...
        skipNewlines()
        val body = parseBlock()
        consume(TokenType.R_BRACE, "Expected '}' after using body")
        consumeNewline()
        return Stmt.UsingContext(values, body, start.line, start.column)
    }
     ...
     *
     * Whether a dotted path names a module or one symbol inside one is left to the
     * semantic passes - only the module graph knows.
     */
    private fun parseUse(exported: Boolean = false, condition: Expr? = null): TopLevel {
        ...
        val specs = mutableListOf<ImportSpec>()
        do {
            specs.add(parseImportSpec())
        } while (match(TokenType.COMMA) && check(TokenType.IDENTIFIER))
        consumeNewline()
        return TopLevel.UseImport(specs, start.line, start.column, exported = exported, condition = condition)
    }

    /**
     * One import clause, joined to [prefix] when it is a member of a group.
     *
     * A member carries its own fully-joined path, so nothing downstream has to
     * track a base path down the tree to read one.
     */
    private fun parseImportSpec(prefix: String = ""): ImportSpec {
        val clauseStart = peek()
        val path = StringBuilder()
        if (prefix.isNotEmpty()) path.append(prefix).append('.')
        ...
        while (match(TokenType.DOT)) {
            when {
                ...
                check(TokenType.L_BRACKET) || check(TokenType.L_BRACE) -> error(
                    "import selector groups use '::{' after the module path; " +
                        "write 'import $path::{…}' at line ${peek().line}",
                )
        ...
        if (match(TokenType.DOUBLE_COLON)) {
            return when {
                ...
                check(TokenType.L_BRACKET) -> error(
                    "import selector groups use braces; write 'import $path::{…}' at line ${peek().line}",
                )
                else -> {
     ...
     * further down with `.` and reach inside again with `::`. Members are separated by
     * a comma or by a line break.
     */
    private fun parseImportGroup(base: String, clauseStart: Token): ImportSpec {
        consume(TokenType.L_BRACE, "Expected '{' to open an import selector group")
        val closer = TokenType.R_BRACE
        val closerText = "}"
        val members = mutableListOf<ImportSpec>()
        skipNewlines()
        if (check(closer)) {
            error("Expected at least one name inside the import group at line ${peek().line}")
        }
        if (check(TokenType.STAR)) {
            return parseWildcardImportGroup(base, clauseStart, closer, closerText)
        }
        if (check(TokenType.WITHOUT)) {
            error(
                "'without' in an import group requires '*' first at line ${peek().line}; " +
                    "write 'import $base::{*, without name}'",
            )
        }
        while (!check(closer) && !isAtEnd()) {
            if (check(TokenType.STAR)) {
                error("'*' must be the first member of a wildcard import group at line ${peek().line}")
            }
            if (check(TokenType.WITHOUT)) {
    ...
    /** `base::{*, without x, without (y, z)}` - wildcard selection with exclusions. */
    private fun parseWildcardImportGroup(
        base: String,
        clauseStart: Token,
        closer: TokenType,
        closerText: String,
    ): ImportSpec {
        val star = consume(TokenType.STAR, "Expected '*' in wildcard import group")
        val excluded = mutableListOf<String>()
        val seen = linkedSetOf<String>()
        var previousLine = star.line
        ...
        while (!check(closer) && !isAtEnd()) {
            ...
            if (match(TokenType.COMMA)) {
                skipNewlines()
            } else if (!separatedByNewline) {
                error("Expected ',' or a new line after '*' or an import exclusion at line ${peek().line}")
            }

            consume(
                TokenType.WITHOUT,
                "A wildcard import group may contain only 'without name' after '*'",
            )
            val names = parseImportExclusionNames()
            for (name in names) {
                if (!seen.add(name)) {
                    error("Import exclusion '$name' is written more than once at line ${peek().line}")
                }
                excluded.add(name)
            }
            previousLine = tokens.getOrNull(current - 1)?.line ?: peek().line
        }

        consume(closer, "Expected '$closerText' after the wildcard import group")
        return ImportSpec(
            base,
            ImportSpec.Selector.All,
            without = excluded,
            line = clauseStart.line,
            column = clauseStart.column,
        )
    }

    /** The name or parenthesized name list following `without` in an import group. */
    private fun parseImportExclusionNames(): List<String> {
        if (!match(TokenType.L_PAREN)) {
            return listOf(consumeIdentifierLike("Expected a symbol name after 'without'"))
        }
        skipNewlines()
        if (check(TokenType.R_PAREN)) {
            error("An import exclusion group cannot be empty at line ${peek().line}")
        }
        val names = mutableListOf<String>()
        while (!check(TokenType.R_PAREN) && !isAtEnd()) {
            names.add(consumeIdentifierLike("Expected a symbol name in the 'without' group"))
            val previousLine = tokens.getOrNull(current - 1)?.line ?: peek().line
            val separatedByNewline = check(TokenType.NEWLINE) || peek().line > previousLine
            if (check(TokenType.NEWLINE)) skipNewlines()
            when {
                ...
                else -> error("Expected ',' or a new line between excluded import names at line ${peek().line}")
            }
        }
        consume(TokenType.R_PAREN, "Expected ')' after excluded import names")
        if (names.size < 2) {
            error("A single import exclusion is written 'without ${names.single()}', without parentheses")
        }
        return names
    }
    ...
    private fun parseReceiverList(): Pair<PropReceiver, List<Param>> {
        val shorthand = parseSelfReceiverShorthand()
        val name: String
        val type: TypeRef
        val modifier: ParamModifier
    ...
    /**
     * `[self&]`, `[self!]`, `[self]`, `[&]`, `[!]` - the receiver, without its type.
     *
     * Inside a body that knows what `Self` is - an `impl`, or a `spec`, whose
     * members are written against whatever implements it - the type is never in
     * question, and the sigil is the only thing that varies. `[&]` and `[!]`
     * name no receiver at all: the member takes one and does not use it.
     *
     * Returns null where `Self` is not settled, which leaves the long form as
     * the only way to write a receiver on a free function.
     */
    private fun parseSelfReceiverShorthand(): PropReceiver? {
        if (currentImplTypeName == null) return null
        val named = check(TokenType.IDENTIFIER) && peek().lexeme == SELF_RECEIVER
        val sigilAt = if (named) 1 else 0
        val modifier = when (tokens.getOrNull(current + sigilAt)?.type) {
            TokenType.AMP -> ParamModifier.SHARED
            TokenType.BANG -> ParamModifier.EXCLUSIVE
            // `[self]` alone consumes; `[]` names no receiver at all.
            else -> if (named) ParamModifier.NONE else return null
        }
        val after = tokens.getOrNull(current + sigilAt + if (modifier == ParamModifier.NONE) 0 else 1)?.type
        if (after != TokenType.R_BRACKET && after != TokenType.COMMA) return null
        repeat(sigilAt + if (modifier == ParamModifier.NONE) 0 else 1) { advance() }
        return PropReceiver(
    ...
    /** `&.member`, `!.member`, or `.member` inside an impl/spec body. */
    private fun parseImplicitMemberReceiver(): PropReceiver? {
        val modifier = when {
            match(TokenType.AMP) -> {
                consume(TokenType.DOT, "Expected '.' after '&' in a shared receiver")
                ParamModifier.SHARED
            }
            match(TokenType.BANG) -> {
                consume(TokenType.DOT, "Expected '.' after '!' in a mutable receiver")
                ParamModifier.EXCLUSIVE
            }
            match(TokenType.DOT) -> ParamModifier.NONE
            else -> return null
        }
        return PropReceiver(
            "self",
    ...
    /** `(name: Type&, other: Other&).member` or `(Type&, Other&).member`. */
    private fun parseExplicitMemberReceiver(): PropReceiver? {
        if (!check(TokenType.L_PAREN)) return null
        var closing = current
        var depth = 0
        while (closing < tokens.size) {
            when (tokens[closing].type) {
                TokenType.L_PAREN -> depth++
                TokenType.R_PAREN -> {
                    depth--
                    if (depth == 0) break
        ...
        if (tokens.getOrNull(closing + 1)?.type != TokenType.DOT) return null
        advance()
        data class ParsedReceiver(val name: String, val type: TypeRef, val modifier: ParamModifier, val borrowWritten: Boolean)
        val receivers = mutableListOf<ParsedReceiver>()
        var unnamed = false
        var index = 0
        do {
            skipNewlines()
            val named = check(TokenType.IDENTIFIER) && peekNext()?.type == TokenType.COLON
            val name = if (named) {
                val n = consumeIdentifierLike("Expected receiver name")
                consume(TokenType.COLON, "Expected ':' after receiver name")
                n
            } else {
                unnamed = true
                "__receiver${index}"
            ...
            val parsed = parseTypeName()
            val (type, modifier, borrowWritten) = if (parsed is TypeRef.Reference) {
                Triple(parsed.inner, parsed.kind.paramModifier, true)
            } else {
                Triple(parsed, ParamModifier.NONE, false)
            }
            receivers.add(ParsedReceiver(name, type, modifier, borrowWritten))
            index++
            skipNewlines()
        } while (match(TokenType.COMMA))
        if (receivers.any { it.name.startsWith("__receiver") } && receivers.any { !it.name.startsWith("__receiver") }) {
            error("a receiver tuple is either fully named or fully unnamed at line ${peek().line}")
        }
        val duplicateType = receivers.groupBy { it.type.displayName() }.entries.firstOrNull { it.value.size > 1 }
        if (duplicateType != null) {
            error("context receivers cannot repeat type '${duplicateType.key}' at line ${peek().line}")
        }
        consume(TokenType.R_PAREN, "Expected ')' after extension receiver")
        consume(TokenType.DOT, "Expected '.' between extension receiver and member name")
        val first = receivers.firstOrNull() ?: error("a receiver tuple cannot be empty at line ${peek().line}")
        pendingMemberContextReceivers = receivers.drop(1).map {
            Param(it.name, it.type, modifier = it.modifier)
        }
        return PropReceiver(
            if (unnamed && receivers.size == 1) "self" else first.name,
            first.type,
            first.modifier,
            borrowWritten = first.borrowWritten,
        )
    }
    ...
    private fun parseTypedMemberReceiver(): PropReceiver? {
        ...
        // path segment ambiguous with the member name.
        ...
        while (i < tokens.size) {
            when (tokens[i].type) {
                ...
                TokenType.L_PAREN, TokenType.NEWLINE, TokenType.EOF -> break
                else -> {}
            }
            i++
        }
    ...
    private fun parsePropAsFin(
        ...
        val prefixReceiver = parseExplicitMemberReceiver() ?: parseTypedMemberReceiver()
        if (pendingMemberContextReceivers.isNotEmpty()) {
            pendingMemberContextReceivers = emptyList()
            error("a property has one observational receiver; use a function for multiple context receivers at line ${start.line}")
        }
     ...
     *
     * ```azora
     * deepinline prop<...T> promote: Type where T.length >= 2 && T is Number {
     *     var result: Type = T.0
    ...
    private fun parseTypeProp(scopePrefix: String = "") {
        ...
        if (!check(TokenType.LESS)) {
            error("type parameters follow '$kind': write 'deepinline $kind<T> name', at line ${peek().line}")
        }
        val afterParams = indexAfterBalanced(current, TokenType.LESS, TokenType.GREATER)
        val upcomingName = tokens.getOrNull(afterParams)?.lexeme ?: "<type-property>"
        val params = parseTypePropParams(upcomingName)
    ...
    private fun parseTypeFunctionExpr(): TypeFunctionExpr {
        ...
        if (match(TokenType.WHEN)) {
            // `when b { true => T; false => U }` - a scrutinee named before the
            // brace, so `when` reads in type position as it does in value position.
            // Each arm becomes the same condition the conditionless form writes.
            val scrutinee = if (check(TokenType.L_BRACE)) null else {
                val name = consumeIdentifierLike("Expected a name or '{' after 'when'")
                name to (parseTypePropCall(name) ?: TypeFunctionExpr.Reference(name))
            }
            consume(TokenType.L_BRACE, "Expected '{' after 'when' in a type computation")
            skipNewlines()
            // The third element records which boolean literal an arm matched, so an
            // exhaustive `true`/`false` pair can stand in for the `else`.
            val arms = mutableListOf<Triple<TypeFunctionCondition, TypeFunctionExpr, Boolean?>>()
            ...
            while (!check(TokenType.R_BRACE)) {
                if (match(TokenType.ELSE)) {
                    ...
                    val matched = if (scrutinee == null) null else booleanArmLiteral()
                    val condition =
                        if (scrutinee == null) parseTypeFunctionCondition()
                        else typeFunctionArmCondition(scrutinee, matched)
                    consume(TokenType.FAT_ARROW, "Expected '=>' after a 'when' condition")
                    arms.add(Triple(condition, parseTypeFunctionExpr(), matched))
                }
                skipNewlines()
            }
            consume(TokenType.R_BRACE, "Expected '}' after 'when'")
            // `true` and `false` between them leave nothing over, so a `when` on a
            // `Bool` needs no `else` and the last arm is the remaining case.
            val exhaustiveBooleans = fallback == null &&
                arms.mapNotNull { it.third }.toSet() == setOf(true, false)
            val last = fallback
                ?: arms.takeIf { exhaustiveBooleans }?.last()?.second
                ?: error("A 'when' in a type computation must end with 'else' at line ${peek().line}")
            val tested = if (exhaustiveBooleans) arms.dropLast(1) else arms
            // Folded from the last arm outwards, so the first written arm is the
            // outermost test and order of writing is order of trying.
            return tested.foldRight(last) { (condition, value, _), rest ->
    ...
    /** The boolean an arm of a scrutinee `when` matches, or null if it names a type. */
    private fun booleanArmLiteral(): Boolean? = when {
    ...
    /**
     * One arm of `when <scrutinee> { … }`, as the condition it stands for.
     *
     * A `true`/`false` arm says which way a `Bool` parameter went, which is the
     * flag test [parseTypeFunctionCondition] already builds for the conditionless
     * form - so the two spellings meet at the same condition and the evaluator
     * learns nothing new. Any other arm names a type the scrutinee is compared to.
     */
    private fun typeFunctionArmCondition(
        scrutinee: Pair<String, TypeFunctionExpr>,
        matched: Boolean?,
    ): TypeFunctionCondition {
        val (name, expression) = scrutinee
        if (matched != null) {
            return TypeFunctionCondition(
                expression, TokenType.EQUAL_EQUAL, expression,
                compareRank = false,
                valueFlags = listOf(name),
                flagsExpected = listOf(matched),
            )
        }
        val armName = consume(TokenType.IDENTIFIER, "Expected a type, 'true', 'false', or 'else' in a 'when' arm").lexeme
        val arm = parseTypePropCall(armName) ?: TypeFunctionExpr.Reference(armName)
        return TypeFunctionCondition(
            expression, TokenType.EQUAL_EQUAL,
            if (match(TokenType.QMARK)) TypeFunctionExpr.Nullable(arm) else arm,
            compareRank = false,
        )
    }
    ...
    private fun parseNoInline(): Stmt.NoInline {
        ...
        val inner = parseSingleStmt("'noinline'")
    ...
    private fun parseInitializer(expectedType: TypeRef?): Expr {
        ...
        if (check(TokenType.L_PAREN)) {
            ...
            // and the repetition is silently lost.
            //
            // Everything after the `*` is the count: `.() * a.size + b.size` asks
            // for as many elements as the two together, not for that many arrays
            // plus a size. The form builds a value rather than multiplying one, so
            // it does not share multiplication's precedence.
            if (check(TokenType.STAR)) {
                val star = advance()
                return Expr.Binary(call, TokenType.STAR, parseExpr(), star.line, star.column, star.lexeme.length)
            }
            return call
        }
    ...
    private fun parseTupleBinding(start: Token, keyword: String): Stmt {
        ...
        skipNewlines()
        while (!check(TokenType.R_PAREN) && !isAtEnd()) {
            names.add(consumeIdentifierLike("Expected a binding name in tuple destructuring"))
            types.add(if (match(TokenType.COLON)) TypeAnnotation.Explicit(parseTypeName()) else TypeAnnotation.Inferred)
            skipNewlines()
            if (!match(TokenType.COMMA)) break
            skipNewlines()
        }
        consume(TokenType.R_PAREN, "Expected ')' after tuple destructuring pattern")
        if (names.size < 2) error("tuple destructuring needs at least two bindings at line ${start.line}")
        if (names.size != names.distinct().size) {
            error("tuple destructuring binds a name more than once at line ${start.line}")
        }
        ...
        val declarations = names.mapIndexed { index, name ->
            ...
            val access = Expr.TupleAccess(target, index, start.line, start.column)
            when (keyword) {
                "var" -> Stmt.VarDecl(name, types[index], access, start.line, start.column, valueMutable = true)
                "val" -> Stmt.VarDecl(name, types[index], access, start.line, start.column, valueMutable = false)
                "let" -> Stmt.LetDecl(name, types[index], access, start.line, start.column)
                else -> Stmt.FinDecl(name, types[index], access, start.line, start.column)
            }
        }
        pendingStmts.addAll(declarations)
        return temp
    ...
    private fun isTupleAssignmentAhead(): Boolean {
        var i = current
        var depth = 0
        while (i < tokens.size) {
            when (tokens[i].type) {
                ...
                TokenType.R_PAREN -> {
                    depth--
                    if (depth == 0) return tokens.getOrNull(i + 1)?.type == TokenType.EQUAL
                }
                TokenType.NEWLINE, TokenType.EOF -> if (depth == 0) return false
                else -> {}
            }
            i++
        }
    ...
    /** `(x, obj.y) = pair` - one tuple evaluation followed by positional writes. */
    private fun parseTupleAssignment(): Stmt {
        val start = peek()
        consume(TokenType.L_PAREN, "Expected '(' to open tuple assignment")
        val targets = mutableListOf<Expr>()
        do {
            skipNewlines()
            targets.add(parseExpr())
            skipNewlines()
        } while (match(TokenType.COMMA))
        consume(TokenType.R_PAREN, "Expected ')' after tuple assignment targets")
        if (targets.size < 2) error("tuple assignment needs at least two targets at line ${start.line}")
        ...
        val writes = targets.mapIndexed { index, target ->
            ...
            buildAssignment(target, Expr.TupleAccess(tempRead, index, start.line, start.column), start.line, start.column)
        }
    ...
    private fun parseReturnedValue(start: Token): Stmt {
        // `return .Variant` fails the function - but only where the signature
        // says it can. A `prop next[…]: Colour` declares no error set, so its
        // `.Green` is a value of the type it returns, read from context like
        // every other bare case.
        if (check(TokenType.DOT) && peekNext()?.type == TokenType.IDENTIFIER && currentFailSets.isNotEmpty()) {
            advance() // '.'
    ...
    private fun parseExprStmt(): Stmt {
        ...
        return when (opTok.type) {
            ...
            // `target++` / `target--` - desugars to `target = target ± 1`. Every
            // assignable target takes one, the same set `target ?++` and `target +=`
            // already take: a local, a field (`self.allocCount++`), an element
            // (`counts[i]++`), and a pointee (`slot.*++`).
            TokenType.PLUS_PLUS, TokenType.MINUS_MINUS -> {
                advance()
                consumeNewline()
                val op = if (opTok.type == TokenType.PLUS_PLUS) TokenType.PLUS else TokenType.MINUS
                val one = Expr.IntLiteral(1, start.line, start.column)
                val rhs = Expr.Binary(expr, op, one, start.line, start.column)
                buildAssignment(expr, rhs, start.line, start.column)
            }
            else -> {
    ...
    /**
     * The arguments of a call, up to but not including the closing `)`.
     *
     * Every argument list reads the same wherever it is written - a named call, a
     * leading-dot construction, an `alloc` - so each accepts:
     *
     *  - `inline for x in xs { expr }` - one argument per iteration
     *  - `...arr` - the array's elements, spread
     *  - `name: value` - a named argument
     *
     * Arguments are separated by a comma or by a line break. A `(` suppresses
     * newline tokens, so a line break leaves nothing between two arguments: the
     * list runs until the `)` and the comma is optional, which is what makes the
     * multi-line call form read the way it is written.
     */
    private fun parseCallArgumentList(): List<Expr> {
        val args = mutableListOf<Expr>()
        skipNewlines()
        while (!check(TokenType.R_PAREN) && !isAtEnd()) {
            if (check(TokenType.INLINE) && peekNext()?.type == TokenType.FOR) {
                args.add(parseInlineForArgument())
                match(TokenType.COMMA)
                skipNewlines()
                continue
            }
            args.add(
                if (match(TokenType.ELLIPSIS)) {
                    val spread = parseExpr()
                    Expr.Spread(spread, spread.line, spread.column, spread.length)
                } else {
                    val first = parseExpr()
    ...
    /**
     * `inline for x in xs { expr }` in argument position - one argument per
     * iteration, expanded once the iteration is known.
     */
    private fun parseInlineForArgument(): Expr {
        val forStart = advance() // 'inline'
        advance() // 'for'
        ...
        // The `{` opens the loop body, not a trailing lambda on the iterable -
        ...
        return Expr.InlineForArgs(loopName, iterable, bodyExpr, forStart.line, forStart.column)
    }

    /**
     * True when [expr] is a plain dotted name (`std.container.array`), which is
     * what a module path looks like in expression position.
     *
     * Only the shape is checked - whether those segments name a module is the
     * module graph's business, and nothing else can be written to the left of a
     * `::` and mean anything, so a chain that turns out not to name one fails as
     * an undefined symbol rather than as a syntax error.
     */
    private fun isModulePath(expr: Expr): Boolean = when (expr) {
     ...
     *
     * Only a name followed by a reflect target (`<Type>`, `<Type::member>`, `<*>`)
     * reaches here, so an ordinary function called `reflect` is unaffected.
     */
    private fun isReflectName(name: String): Boolean =
        name == "reflect" || name.endsWith("__reflect")
    ...
    private fun parseUnary(): Expr {
        ...
        if (check(TokenType.ALLOC)) {
            ...
            // on the type, where the same sigil sits when the type is written out.
            // `alloc* T(…)` / `alloc^ T(…)` put it on the keyword instead, and are
            // the older spelling.
            var mutable = when {
            ...
            if (check(TokenType.IDENTIFIER) &&
                (peekNext()?.type == TokenType.STAR || peekNext()?.type == TokenType.CARET) &&
                tokens.getOrNull(current + 2)?.type == TokenType.L_PAREN
            ) {
                val typeTok = advance()
                mutable = advance().type == TokenType.CARET
                advance() // '('
                val args = parseCallArgumentList()
                consume(TokenType.R_PAREN, "Expected ')' after 'alloc ${typeTok.lexeme}*(' arguments")
                val call = Expr.Call(typeTok.lexeme, args, typeTok.line, typeTok.column, typeTok.lexeme.length)
                // A trailing `* count` is left to the caller's multiplicative loop,
                // which is where `alloc .(…) * count` already reads it.
                return Expr.Alloc(call, at.line, at.column, at.lexeme.length, mutable = mutable)
            }
            ...
            when {
                ...
                check(TokenType.DOUBLE_COLON) -> {
                    ...
                    expr = when {
                        ...
                        // `std.container.array::Array` - a dotted module path, then
                        // the symbol it declares. The path says which module to look
                        // in; inside it the symbol is declared under its own name,
                        // so that name is what the access resolves to.
                        isModulePath(expr) ->
                            Expr.Identifier(member, expr.line, expr.column, member.length)
                ...
                check(TokenType.LESS) && expr is Expr.Member && expr.name in setOf("hasAnnot", "annotMeta", "withAnnot") -> {
                    ...
                    // `reflect<Type>.field` accesses a member of the reflect handle;
                    // reflected declaration members must go inside the angle brackets
                    // with `::` (`reflect<Type::field>`).
                    if (innerTarget is Expr.Member &&
                        (innerTarget.target as? Expr.Call)?.callee == "__reflect"
                    ) {
                        error("Reflected declaration members use '::', for example 'reflect<Type::${innerTarget.name}>' at line ${expr.line}")
                    }
        ...
        if (!check(close)) {
            do {
                ...
                val arg = if (match(TokenType.ELLIPSIS)) {
                    ...
                    val first = parseExpr()
                    // `@map[1: "one"]` - a keyed argument, for an arm written
    ...
    /** The new single ownership header: `[modes, captures, without names] receivers? { … }`. */
    private fun parseReceiverLambda(
        ...
        val exclusions = mutableListOf<CaptureExclusion>()
        val defaults = mutableListOf<Pair<CaptureMode, Token>>()
        skipNewlines()
        while (!check(TokenType.R_BRACKET) && !isAtEnd()) {
            if (check(TokenType.SEMICOLON)) {
                error(
                    "line ${peek().line}: the '[receivers; captures]' lambda form was removed; " +
                        "use one ownership header, for example '[&] receiver { … }' or '[value.&] { … }'",
                )
            }
            when {
                isCaptureDefaultAhead() -> {
                    val token = advance()
                    val mode = when (token.type) {
                        TokenType.EQUAL -> CaptureMode.COPY
                        TokenType.AMP -> CaptureMode.SHARED
                        TokenType.BANG -> CaptureMode.MUTABLE
                        else -> CaptureMode.MOVE
                    }
                    defaults.add(mode to token)
                }
                check(TokenType.WITHOUT) -> exclusions.addAll(parseLambdaCaptureExclusions())
            ...
            when {
                match(TokenType.COMMA) -> skipNewlines()
                check(TokenType.R_BRACKET) -> Unit
                separatedByNewline -> Unit
                else -> error("Expected ',' or a new line between lambda ownership entries at line ${peek().line}")
            }
        }
        ...
        when {
            check(TokenType.IDENTIFIER) -> receivers.add(parseLambdaReceiverName())
            match(TokenType.L_PAREN) -> {
                skipNewlines()
                do {
                    receivers.add(parseLambdaReceiverName())
                    skipNewlines()
                } while (match(TokenType.COMMA).also { if (it) skipNewlines() })
                consume(TokenType.R_PAREN, "Expected ')' after lambda receiver names")
                if (receivers.size < 2) {
                    error("A single lambda receiver name is written without parentheses at line ${start.line}")
                }
                skipNewlines()
            }
        }
        if (check(TokenType.L_BRACKET)) {
            error(
                "line ${peek().line}: a lambda has only one ownership header; " +
                    "merge capture overrides into the first '[…]' header",
            )
        }

        val default = when {
            defaults.size <= 1 -> defaults.singleOrNull()?.first
            receivers.isEmpty() -> error("line ${defaults[1].second.line}: a lambda has only one capture default")
            defaults.size != receivers.size -> error(
                "line ${start.line}: ${defaults.size} receiver modes cannot describe ${receivers.size} receivers",
            )
            defaults.any { it.first == CaptureMode.COPY } -> error(
                "line ${start.line}: '=' is a capture default, not a receiver capability",
            )
            else -> null // positional mixed receiver modes have no capture default
        }
        val receiverModes = when {
            receivers.isEmpty() || defaults.isEmpty() -> emptyList()
            defaults.size == 1 -> List(receivers.size) { defaults.single().first }
            else -> defaults.map { it.first }
        }
        receiverModes.forEachIndexed { index, mode ->
            receivers[index] = receivers[index].copy(
                modifier = when (mode) {
                    CaptureMode.SHARED -> ParamModifier.SHARED
                    CaptureMode.MUTABLE -> ParamModifier.EXCLUSIVE
                    CaptureMode.MOVE -> ParamModifier.NONE
                    else -> receivers[index].modifier
                },
            )
        }
        if (exclusions.isNotEmpty() && default == null) {
            error("line ${exclusions.first().line}: 'without' requires a capture default such as '[&, without name]'")
        }
        val excludedSources = exclusions.map { it.source }.toSet()
        captures.firstOrNull { it.source in excludedSources }?.let {
            error("line ${it.line}: '${it.source}' cannot be both captured and excluded with 'without'")
        }
        if (default != null) {
            captures.firstOrNull { it.mode == default }?.let {
                error(
                    "line ${it.line}: '${it.source}' repeats the '${default.spelling}' capture default; " +
                        "remove the redundant explicit capture",
                )
            }
        }
        ...
        return lambda.copy(
            ...
            captureExclusions = exclusions,
            kind = kind,
        )
    }

    /** One inferred receiver alias after a lambda ownership header. */
    private fun parseLambdaReceiverName(): Param {
        if (check(TokenType.ELLIPSIS)) {
            error("line ${peek().line}: contextual receivers cannot be variadic")
        }
        val name = consumeIdentifierLike("Expected a lambda receiver name")
        if (!match(TokenType.COLON)) return Param(name, TypeRef.Named("Any", synthesized = true))
        val annotated = parseTypeName()
        if (annotated is TypeRef.Reference) {
            error(
                "line ${peek().line}: receiver ownership belongs in the lambda header; " +
                    "write '[${if (annotated.kind == TypeRef.RefKind.MUTABLE) "!" else "&"}] $name: ${annotated.inner}'",
            )
        }
    ...
    /**
     * One ownership-header entry: `value`, `value.&`, `value.!`, `value.clone()`,
    ...
    private fun parseCapture(): Capture {
        ...
        if (check(TokenType.BANG)) {
            error(
                "line ${start.line}: mutable capture borrows use the dotted ownership spelling; " +
                    "write '$name.!'",
            )
        }
