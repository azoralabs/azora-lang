...
/**
 * End-to-end tests for the [org.azora.lang.backend.SwiftCodegen] backend.
 *
 * Each test compiles a small Azora program to Swift 6.3, compiles it with
 * `swiftc`, executes the resulting binary, and asserts on the program's
 * standard output — validating that the generated Swift is both syntactically
 * valid and semantically matches the interpreter.
 *
 * `swiftc` start-up is expensive, so this suite is deliberately compact and
 * skips itself when no `swiftc` is available.
 */
class SwiftCodegenExecTest {

    /** Runs [source] and asserts its stdout equals [expected] (skips without `swiftc`). */
    private fun check(expected: String, source: String) {
        if (!SwiftExec.available) return
        assertEquals(expected, SwiftExec.run(source))
    }
    ...
    /** Swift integer division truncates toward zero, matching the interpreter. */
