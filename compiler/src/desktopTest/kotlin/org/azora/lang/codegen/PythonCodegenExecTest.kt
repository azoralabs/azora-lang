...
/**
 * End-to-end tests for the [org.azora.lang.backend.PythonCodegen] backend: each
 * compiles a small Azora program to Python, runs it with `python3`, and asserts
 * on stdout. Skips itself when no `python3` is available.
 */
class PythonCodegenExecTest {

    private fun check(expected: String, source: String) {
        if (!PythonExec.available) return
        assertEquals(expected, PythonExec.run(source))
    }
