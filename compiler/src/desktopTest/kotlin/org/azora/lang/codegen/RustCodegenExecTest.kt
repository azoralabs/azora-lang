...
/**
 * End-to-end tests for the [org.azora.lang.backend.RustCodegen] backend: each
 * compiles a small Azora program to Rust, builds it with `rustc`, runs the
 * binary, and asserts on stdout. Skips itself when no `rustc` is available.
 */
class RustCodegenExecTest {

    private fun check(expected: String, source: String) {
        if (!RustExec.available) return
        assertEquals(expected, RustExec.run(source))
    }
    ...
    @Test fun lambdaValue() = check("10", main("var double = { x: Int -> x * 2 }\nprintln(double(5))"))

    @Test fun higherOrderFunction() = check(
        "16",
        """
        func apply(f: (Int) -> Int, x: Int): Int { return f(x) }
        func main() { println(apply({ x: Int -> x * x }, 4)) }
        """.trimIndent()
    )
    ...
    /** Awaited spawned tasks yield deterministic results (tasks lower synchronously). */
