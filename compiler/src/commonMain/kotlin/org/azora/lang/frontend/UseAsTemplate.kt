...
/**
 * Expands a compact prot `use as` member-name template.
 *
 * The template text is ordinary text. `${T}`, `${T.typeName}`, and `${T.name}`
 * refer to the type argument bound to prot type parameter `T`. The expanded
...
internal object UseAsTemplate {
        ...
        while (i < template.length) {
            ...
            if (ch == '$' && i + 1 < template.length && template[i + 1] == '{') {
                val end = findPlaceholderEnd(template, i + 2)
                    ?: throw IllegalStateException("Unclosed placeholder in prot 'use as' template: $template")
    ...
    private fun resolvePlaceholder(expr: String, typeParams: List<String>, traitArgs: List<TypeRef>): String {
        ...
        if (parts.isEmpty()) {
            throw IllegalStateException("Empty placeholder in prot 'use as' template")
        }
        val param = parts.first()
        val index = typeParams.indexOf(param)
        if (index < 0) {
            throw IllegalStateException("Unknown type parameter '$param' in prot 'use as' template")
        }
        ...
        return when (val property = parts.drop(1).joinToString(".")) {
            ...
            else -> throw IllegalStateException("Unknown type template property '$property' in prot 'use as' template")
        }
    }
}
